            ...
            <div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="bio" id="bio" rows="3" 
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('bio') }}</textarea>
                </div>

                <!-- Avatar -->
                <!-- Avatar using Media Picker -->
                <div class="mt-6">
                    ...
                    <x-admin.media-picker 
                        ...
                        :value="old('avatar', '')"
                        collection="users"
                        help="اختر صورة شخصية للمستخدم من مكتبة الوسائط أو ارفع صورة جديدة. يُفضل أن تكون مربعة الشكل" />
                    @error('avatar')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($roles as $id => $name)
                            <div class="flex items-center">
                                <input type="checkbox" name="roles[]" value="{{ $name }}" id="role_{{ $id }}"
                                       {{ in_array($name, old('roles', [])) ? 'checked' : '' }}
